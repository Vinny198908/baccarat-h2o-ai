...
app = FastAPI(title="Baccarat H2O Top Tier Road Intelligence", version="5.0")
app.add_middleware(
    CORSMiddleware,
...
_bot_state: dict[str, Any] = {"version":"ultimate-v3","predictions":0,"resolved":0,"wins":0,"losses":0,"waits":0,"by_phase":{},"by_model":{},"pending":None,"recent":[]}
...
def health() -> dict[str, Any]:
    return {
        "ok": True,
        "version": "5.0-ultimate-bot-no-deepseek",
        "model_id": _model_id,
...
def _road_vote(row: dict[str, Any]) -> dict[str, Any]:
    p_red = int(row.get("preview_p_red_count", 0)); b_red = int(row.get("preview_b_red_count", 0))
    # Derived roads are structure signals, not direct P/B outcomes. Compare hypothetical next-road structure.
    if p_red == b_red:
        return {"name":"roads","side":None,"confidence":0.5,"evidence":f"preview red {p_red}-{b_red}"}
    side = "P" if p_red > b_red else "B"
    conf = min(0.62, 0.51 + 0.035 * abs(p_red-b_red))
    return {"name":"roads","side":side,"confidence":conf,"evidence":f"preview red P={p_red} B={b_red}"}


def _sequence_vote(seq: str, row: dict[str, Any]) -> dict[str, Any]:
    recent = [x for x in seq[-12:] if x in "PB"]
    if len(recent) < 5: return {"name":"sequence","side":None,"confidence":0.5,"evidence":"insufficient recent hands"}
    p = recent.count("P"); b = recent.count("B")
    if p == b: return {"name":"sequence","side":None,"confidence":0.5,"evidence":"balanced recent sequence"}
    side = "P" if p>b else "B"
    return {"name":"sequence","side":side,"confidence":min(0.60, 0.5 + abs(p-b)/(2*len(recent))),"evidence":f"last {len(recent)} P={p} B={b}"}


def _ultimate_decision(seq: str, use_kaggle: bool = True) -> dict[str, Any]:
    started = time.perf_counter(); row = features_for_sequence(seq); phase = _phase(len(seq)); votes=[]
    votes.append(_road_vote(row)); votes.append(_sequence_vote(seq,row))
    library = None
    if use_kaggle and time.perf_counter()-started < BOT_DECISION_BUDGET_SECS-0.25:
        try:
            library = compact_library_score(seq)
            lp, lb = float(library.get("p",0)), float(library.get("b",0))
            side = "P" if lp>lb else "B" if lb>lp else None
            votes.append({"name":"library","side":side,"confidence":max(lp,lb),"evidence":f"matches={library.get('matches',0)} suffix={library.get('suffix_length',0)}"})
        except Exception as exc:
            votes.append({"name":"library","side":None,"confidence":0.5,"error":str(exc)[:120]})
    if _model is not None and time.perf_counter()-started < BOT_DECISION_BUDGET_SECS-0.25:
        try:
            ensure_h2o(); hf=h2o.H2OFrame(pd.DataFrame([row],columns=FEATURE_COLUMNS))
            for col in CATEGORICAL: hf[col]=hf[col].asfactor()
            hp=prediction_probabilities(_model,hf,_model_classes); side="P" if hp["P"]>hp["B"] else "B"
            votes.append({"name":"h2o","side":side,"confidence":max(hp["P"],hp["B"]),"evidence":f"P={hp['P']:.3f} B={hp['B']:.3f}"})
        except Exception as exc: votes.append({"name":"h2o","side":None,"confidence":0.5,"error":str(exc)[:120]})
    scores={"P":0.0,"B":0.0}; active=0
    for v in votes:
        if v.get("side") not in scores: continue
        rel=_model_reliability(v["name"],phase); v["reliability"]=round(rel,4)
        # confidence above chance × learned reliability; caps prevent one model dominating.
        weight=max(0.01,min(0.30,(float(v.get("confidence",.5))-.5)*2))* (0.75+rel)
        scores[v["side"]]+=weight; active+=1
    total=scores["P"]+scores["B"]
    agreement=(max(scores.values())/total) if total else .5
    side=max(scores,key=scores.get) if total else None
    elapsed=time.perf_counter()-started
    min_agree = .61 if phase in ("40-54","55+") else .59
    decision=side if side and active>=2 and agreement>=min_agree and elapsed<=BOT_DECISION_BUDGET_SECS else "WAIT"
    confidence=agreement if decision!="WAIT" else .5
    return {"decision":decision,"confidence":round(confidence,4),"phase":phase,"hand_count":len(seq),"decision_seconds":round(elapsed,4),"budget_seconds":BOT_DECISION_BUDGET_SECS,"models_complete":len(votes),"votes":votes,"scores":scores,"library":library}
...
def bot_decide(payload: dict[str, Any], request: Request) -> dict[str, Any]:
    ...
    result=_ultimate_decision(seq,use_kaggle)
    result["library_mode"]=library_mode
    result["shoe_color"]=shoe_color
    with _bot_lock:
        _bot_state["predictions"]=int(_bot_state.get("predictions",0))+1
        if result["decision"]=="WAIT": _bot_state["waits"]=int(_bot_state.get("waits",0))+1; _bot_state["pending"]=None
        else: _bot_state["pending"]={"sequence":seq,"prediction":result["decision"],"phase":result["phase"],"votes":result["votes"],"created_at":time.time(),"library_mode":library_mode,"shoe_color":shoe_color}
        _save_bot_state()
...
def bot_feedback(payload: dict[str, Any], request: Request) -> dict[str, Any]:
    ...
    with _bot_lock:
        ...
        recent=_bot_state.setdefault("recent",[]); recent.append({"prediction":pred,"actual":actual,"win":win,"phase":phase,"ts":time.time()}); _bot_state["recent"]=recent[-250:]
...
def bot_status(request: Request) -> dict[str, Any]:
    ...
    return {"live":True,"version":"ultimate-v4-combined-no-deepseek","decision_budget_seconds":BOT_DECISION_BUDGET_SECS,"accuracy":round(wins/resolved,4) if resolved else None,"state":_bot_state,"model_id":_model_id}
